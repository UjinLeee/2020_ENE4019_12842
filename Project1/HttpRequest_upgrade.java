...
final class HttpRequest_upgrade implements Runnable {
    ...
    public void run() {
        try {
            ...
            System.out.println("Exception occured while processing request: "); //change
            e.printStackTrace();
        }
    }
    ...
    private void processRequest() throws Exception {
       ...
       // InputStream is = /*socket.getInputStream()*/ null;
        InputStream is = null;
    ...
    private String parseRequestLine(String requestLine) {
        ...
        if (tokens.countTokens() != 3) {
            code = StatusCode.NOT_FOUND;
            return "Request line is malformed. Returning BAD Bot Found.";
        }
        ...
        if (file.isDirectory()) {
            ...
            if (list == null || list.length == 0) {
                code = StatusCode.NOT_FOUND;
                return "No index file found at requested location " + fileName + " Returning Nor Found";
    ...
    private void sendResponseMessage(FileInputStream fis, DataOutputStream os) throws Exception {
        ...
        if (code != StatusCode.OK)
            entityBody = entityBody.replaceAll("\\?", message + " - sent by Yujin's webServer");
